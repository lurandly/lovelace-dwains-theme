...
class DwainsNotificationCard extends LitElement {
  ...
  async _subcribeNotifications(){
    if (!this._unsub) {
      this._unsub = await this.hass.connection.subscribeEvents(() => this._notificationsUpdated(), "dwains_theme_notifications_updated");
    }
  }
  ...
  async _notificationsUpdated(){
    this.notifications = await this.hass.callWS({
      type: 'dwains_theme_notification/get'
    });
  }

  _handleDismiss(event) {
    var id = event.target.notificationId;
     this.hass.callService("dwains_theme", "notification_dismiss", {
       notification_id: id
     });   
   }
